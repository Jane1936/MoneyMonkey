...
function toWeekKey(dateStr) {
  ...
  const week = 1 + Math.floor((date - firstThursday) / (7 * 24 * 3600 * 1000));
  return `${isoYear}-W${String(week).padStart(2, '0')}`;
}

function normalizeTransaction(item) {
  const buyCurrency = CURRENCY_OPTIONS.includes(item.buyCurrency) ? item.buyCurrency : 'RMB';
  const sellCurrency = CURRENCY_OPTIONS.includes(item.sellCurrency) ? item.sellCurrency : buyCurrency;
  return {
    ...item,
    stockName: item.stockName || '未命名标的',
    buyCurrency,
    sellCurrency,
    buyAmount: Number(item.buyAmount || 0).toFixed(2),
    sellAmount: Number(item.sellAmount || 0).toFixed(2),
    returnRate: Number(item.returnRate || 0).toFixed(2)
  };
}
...
function calculatePeriodReport(rawTransactions, getPeriodKey) {
  const map = {};
...
function generateCurveData(rawTransactions, granularity = 'month') {
